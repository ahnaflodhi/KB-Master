...
| `commands/_delegate.md` | **shipped** (v2.8, spec-only) | commands/ | Eleven-step dispatch sequence specified. Step 3 PREPARE will rewire to bundle-loading in v3.0 Phase 5. Step 2 PROBE + Step 4 DISPATCH will gain host_access checks in Phase 5. |
| `commands/pre-check.md` | **shipped** (v1.0+, drift-fixed v2.8.1) | commands/ | Only role-bearing slash command currently in repo. |
| 10 remaining role-bearing slash commands (`plan`, `audit`, `execute`, `evaluate`, `kb-lint`, `wiki-ingest`, `wiki-query`, `escalate`, `meta-review`, `apply-meta`) | **planned** (v3.0 Phase 5) | commands/ | Tracked as long-standing gap (CHANGELOG v2.1, project CLAUDE.md). Phase 3 produced the corresponding `20-roles/<role>.md` contracts that these commands will reference. |
...
| Bundle manifests (`bundles/*.yaml`) | **planned** (v3.0 Phase 5) | bundles/ | 13 manifests (one per role + `agent-onboarding`). Audience+also_needed_by frontmatter on Layer-2 files now ready to drive `tools/build-bundle.sh`. |
...
| INDEX.md (Layer-3 entrypoint) | **planned** (v3.0 Phase 5) | project root | Will replace SYSTEM-BLUEPRINT.md as agent ingest entry. |
...
| **5 — Adoption + status + bundles + commands wiring** | **planned** | — |
