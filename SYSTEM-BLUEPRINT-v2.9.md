...
**Version**: 2.9 | **Owner**: KB-Orchestrator-Core (Claude Code)
...
|---|---|---|---|
| `probe` | — | `{available: bool, protocol: int, capabilities: [...], enforces_pre_action_facts: bool \| "orchestrator-side"}` | Run at orchestrator startup. Failure → adapter marked unavailable; agents bound to it cannot be assigned. The `enforces_pre_action_facts` field (Invariant 10, v2.9) is REQUIRED — adapters reporting `false` may only fulfil read-only roles. `"orchestrator-side"` means the orchestrator emits the §25-mandated pre-action fact block on the adapter's behalf before each dispatch (used by the codex-bridge MVP until bridge protocol ≥ 2 ships an in-process callback). |
